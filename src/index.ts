...
class HomebridgeIotas {
    ...
    updateFeature(featureId: string, value: CharacteristicValue) {
        ...
        return this.withAuth().then((api) =>
            api.put(`/feature/${featureId}/value`, body).then((response) => {
    ...
    initServices(accessory: PlatformAccessory) {
        ...
        for (const feature of device.features ?? []) {
            ...
            if (
                ...
                [, "Light", "Operation Mode"].includes(feature.featureTypeName)
            ...
            } else if (feature.featureTypeCategory === "lock") {
                let service = accessory.getService(this.Service.LockMechanism);
                if (typeof service === "undefined") {
                    service = new this.Service.LockMechanism(accessory.displayName);
                    accessory.addService(service);
                }
                service
            ...
            } else if (feature.eventTypeName === "Temperature" && device.category != "lock") {
    ...
    tryAddDevice(roomName: string, device: Device) {
        if (
            !device.features.some(
                (feature) =>
                    ["Temperature", "OnOff", "Level", "ThermostatMode", "Lock"].includes(
